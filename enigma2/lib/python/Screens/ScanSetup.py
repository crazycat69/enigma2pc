...
class ScanSetup(ConfigListScreen, Screen, CableTransponderSearchSupport):
	...
	def createConfig(self, frontendData):
			...
			self.scan_sat.is_id = ConfigInteger(default = defaultSat.get("is_id",0), limits = (0, 256))
			...
			if self.scan_type.value == "single_transponder":
				...
				if len(nimsats):
					...
					if len(tps) and len(tps) > self.preDefTransponders.index:
						...
						self.addSatTransponder(tlist, tp[1] / 1000, tp[2] / 1000, tp[3], tp[4], tp[7], orbpos, tp[5], tp[6], tp[8], tp[9], tp[10], tp[11], tp[10])
				removeAll = False
			elif self.scan_type.value == "single_satellite":
				sat = self.satList[index_to_scan][self.scan_satselection[index_to_scan].index]
